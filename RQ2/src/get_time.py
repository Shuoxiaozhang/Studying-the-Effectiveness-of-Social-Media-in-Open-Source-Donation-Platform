...
slugs_df = pd.read_csv('E:/bishe/newdata/collectiveinfo/updated_merged_collective_data.csv', encoding='utf-8')
...
def save_to_file(batch_data, filename):
    with open(filename, 'a', encoding='utf-8') as file:
        if batch_data:
            # 写入数据时使用逗号连接每个对象，确保是一个有效的 JSON 数组
            json.dump(batch_data, file, ensure_ascii=False, indent=4)
            file.write("\n")  # 每次保存一批数据，换行

# 打开文件，准备保存查询成功的结果
success_file = 'E:/bishe/newdata/collectiveinfo/success_collective_data_update.json'
failed_file = 'E:/bishe/newdata/collectiveinfo/failed_slugs_lost_update.csv'
